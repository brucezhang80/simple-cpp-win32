...
inline bool CheckMemoryBreakpointDebugger()
{
...
//
inline bool	IsInVMMatrix()
{
	bool	bInMatrix	= false;
	HANDLE	hProcess	= GetCurrentProcess();

	LPVOID	lpAddress	= VirtualAllocEx(hProcess, NULL, 6, MEM_RESERVE|MEM_COMMIT , PAGE_EXECUTE_READWRITE);
	if(NULL	== lpAddress)
	{
		return	false;
	}
...
inline bool	IsParentSameImage()
{
	...
	HANDLE parent_handle = OpenProcess(PROCESS_QUERY_INFORMATION, FALSE,	parent_pid);
	...
	{
		GetProcessImageFileName(GetCurrentProcess(), self, sizeof(self));
		GetProcessImageFileName(parent_handle, parent, sizeof(parent));
	}

	CloseHandle(parent_handle);
	return (0 == strcmp(self, parent));
}
#endif // AntiRE_h__